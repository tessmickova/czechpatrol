...
interface Radek {
  ...
  kdy: string | null;
  titulek: string;
  stitek: "nepotvrzeno" | "zachyceno" | "neověřeno" | null;
  pruh: string;
  /* Do rozbalení. */
...
function Sloupec({ nadpis, ikona, ton, radky, prazdne, paticka, ted }: { nadpis: string; ikona: "fajfka" | "otaznik"; ton: "klid" | "pozor"; radky: Radek[]; prazdne: string; paticka?: React.ReactNode; ted: number }) {
...
export function AktualitySloupce({ zaznamy, nepotvrzene = [], kandidati = [], ted = Date.now() }: { zaznamy: Zaznam[]; nepotvrzene?: Zaznam[]; kandidati?: Kandidat[]; ted?: number }) {
  ...
  const neoverene: Radek[] = [
    ...zaznamy.filter((z) => z.overeni === "neovereno").map((z) => zZaznamu(z, `u-${z.slug}`, `/incident/${z.slug}/`, "neověřeno", "bg-jantar")),
