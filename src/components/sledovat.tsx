  ...
  return (
    <section id="sledovat" aria-label="Sledujte nás" className="scroll-mt-[64px]">
      ...
      <ul className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <li>
          {UCTY_ZAPNUTE ? (
            <Link href="/ucet/" className={`${KARTA} border-akcent/50 bg-akcent/10 hover:bg-akcent/20`}><ZnackaKanalu znacka="email" velikost={26} /><span><span className="block text-[13.5px] font-bold text-inkoust">Souhrn a upozornění</span><span className="block text-[11.5px] text-tlum">týdně, nebo hned při změně</span></span></Link>
          ) : (
            <span className={`${KARTA} border-dashed border-linka opacity-80`}><ZnackaKanalu znacka="email" velikost={26} tlumena /><span><span className="block text-[13.5px] font-bold text-tlum">E-mailový souhrn</span><span className="block text-[11.5px] text-tlum2">připravujeme</span></span></span>
          )}
        </li>
        {KANALY_DEF.map((k) => {
          ...
          return (
            <li key={k.klic}>
              {url ? (
                <a href={url} target="_blank" rel="noopener noreferrer" className={`${KARTA} border-linka hover:border-akcent`}><ZnackaKanalu znacka={k.klic} velikost={26} /><span><span className="block text-[13.5px] font-bold text-inkoust">{k.nazev}</span><span className="block text-[11.5px] text-tlum">{k.popis}</span></span></a>
              ) : (
                <span className={`${KARTA} border-dashed border-linka opacity-80`}><ZnackaKanalu znacka={k.klic} velikost={26} tlumena /><span><span className="block text-[13.5px] font-bold text-tlum">{k.nazev}</span><span className="block text-[11.5px] text-tlum2">připravujeme</span></span></span>
              )}
            </li>
          );
        })}
...
export function Partneri() {
  ...
  return (
    <section aria-label="Ve spolupráci s">
      <div className="mb-1.5 flex items-center justify-between"><span className="stitek">Ve spolupráci s</span><span className="text-[11.5px] text-tlum2">partnerství nemění hodnocení ani data</span></div>
          ...
          <li key={i}>
            {p ? (
              <a href={p.url} target="_blank" rel="noopener noreferrer" className={`${KARTA} justify-center border-linka bg-plocha hover:border-akcent`}>
                <span className="text-center"><span className="block text-[14px] font-bold text-inkoust">{p.nazev}</span><span className="block text-[11.5px] text-tlum">{p.popis}</span></span>
              </a>
            ) : (
              <span className={`${KARTA} justify-center border-dashed border-linka`}>
                <span className="text-center text-[12px] text-tlum2">volné místo<br />pro partnera</span>
              </span>
            )}
          </li>
        ))}
