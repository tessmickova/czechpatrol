      ...
      <div className="overflow-x-auto">
        <svg viewBox={`0 0 ${SIRKA} ${NAHORE + V + DOLE}`} role="img" aria-label="Počet jedinečných případů po měsících" className="h-[150px] w-full min-w-[640px]">
          ...
          {kroky.map((k) => {
            ...
            return (
              <g key={k}>
                <line x1={LEVO} x2={SIRKA - PRAVO} y1={y} y2={y} stroke="#f4f3f1" strokeWidth="1" />
                <text x={LEVO - 6} y={y + 4} textAnchor="end" fontSize="11" fontFamily="var(--font-mono)" fill="#3d3b35">{k}</text>
              </g>
            );
          })}
          {rada.map((m, i) => {
            ...
            const popisek = mm === 1 && (rada.length < 60 || y % 2 === 0) && (
              <text x={x0 + 2} y={NAHORE + V + 18} fontSize="11" fontFamily="var(--font-mono)" fill="#6f6c64">{y}</text>
            );
            if (m.pripady === null) {
            ...
            return (
              <g key={m.mesic}>
                <rect x={x0 + 0.5} y={NAHORE + V - h} width={Math.max(1, S - 1)} height={h} fill="#c1272d" rx="1">
