...
export function Vykricniky({ n }: { n: number }) {
  ...
  if (!k) return null;
  return (
    <span aria-label={`${k === 3 ? "velmi vysoký" : k === 2 ? "vysoký" : "zvýšený"} počet signálů`} className="inline-flex items-center gap-[1px] text-[#c1272d]">
...
function ZnackaTrendu({ nyni, drive }: { nyni: Uroven; drive?: Uroven }) {
  ...
  return (
    <span className={`flex items-center gap-1 text-[12.5px] font-semibold ${r > 0 ? "text-[#a8401a]" : "text-[#256b45]"}`}>
...
const RADY: { klic: "celkova" | "hybridni" | "primyStret"; nazev: string; barva: string; silna: boolean }[] = [
  { klic: "celkova", nazev: "Celková úroveň", barva: "#14140f", silna: true },
  { klic: "hybridni", nazev: "Hybridní tlak", barva: "#d1521f", silna: false },
  { klic: "primyStret", nazev: "Přímý střet", barva: "#2e7d53", silna: false },
];
        ...
        <svg viewBox={`0 0 ${SIRKA} ${NAHORE + V + DOLE}`} role="img" aria-label="Vývoj hodnocení po týdnech od začátku roku" className="h-[230px] w-full min-w-[560px]">
          <defs>
            <pattern id="srafy-bez-dat" width="8" height="8" patternUnits="userSpaceOnUse" patternTransform="rotate(-45)">
              <line x1="0" y1="0" x2="0" y2="8" stroke="#6f6c64" strokeWidth="1" strokeOpacity="0.35" />
            </pattern>
            <linearGradient id="plocha-celkem" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#14140f" stopOpacity="0.14" />
          ...
          {prvniSData > 0 && (
            <>
              ...
              <text x={(x(0) + x(prvniSData)) / 2} y={NAHORE + V / 2} textAnchor="middle" fontSize="13" fontFamily="var(--font-mono)" fill="#6f6c64">bez doložených dat</text>
            </>
          )}
          {popisky.map((p) => {
            ...
            return (
              <g key={p.poradi}>
                <line x1={LEVO - 8} x2={SIRKA - PRAVO} y1={yy} y2={yy} stroke="#f4f3f1" strokeWidth="1" />
                <text x={LEVO - 14} y={yy + 4.5} textAnchor="end" fontSize="13" fontFamily="var(--font-mono)" fill="#3d3b35">{p.text}</text>
              </g>
            );
          })}
          ...
          {sloty.map((s, i) => {
            ...
            return (
              <g key={s.od}>
                {prvniVMesici && (
                  <text x={x(i)} y={NAHORE + V + 20} textAnchor="middle" fontSize="12" fontFamily="var(--font-mono)" fill="#3d3b35">{mesice[d.getUTCMonth()]}</text>
                )}
                {s.tyden && (
                  <>
                    ...
                    {RADY.map((r) => (
                      <circle key={r.klic} cx={x(i)} cy={y(s.tyden![r.klic])} r={r.silna ? 3 : 2.2} fill="#fffefb" stroke={r.barva} strokeWidth={r.silna ? 2 : 1.4} />
                    ))}
                  </>
                )}
              </g>
        ...
        <svg viewBox={`0 0 ${SIRKA} ${NAHORE + V + DOLE}`} role="img" aria-label="Měsíční úroveň od roku 2013" className="h-[124px] w-full min-w-[560px]">
          <defs>
            <pattern id="srafy-mesice" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(-45)">
              <line x1="0" y1="0" x2="0" y2="6" stroke="#6f6c64" strokeWidth="1" strokeOpacity="0.3" />
            </pattern>
          </defs>
          {mesice.map((m, i) => {
            ...
            const tecka = m.zaznamu > 0 && (
              <circle cx={x0 + S / 2} cy={NAHORE + V - 6 - Math.min(4, m.zaznamu) * 6} r={Math.min(4.5, 2 + m.zaznamu * 0.6)} fill={m.nejvyssi ? tokeny(m.nejvyssi).plna : "#3d3b35"} stroke="#fffefb" strokeWidth="1">
            ...
            if (!m.uroven) {
              return (
                <g key={m.mesic}>
                  ...
                  {tecka}
                  {mm === 1 && (y % 2 === 0) && <text x={x0 + 2} y={NAHORE + V + 18} fontSize="11" fontFamily="var(--font-mono)" fill="#6f6c64">{y}</text>}
                </g>
              );
            }
            ...
            return (
              <g key={m.mesic}>
                ...
                {tecka}
                {mm === 1 && (y % 2 === 0) && <text x={x0 + 2} y={NAHORE + V + 18} fontSize="11" fontFamily="var(--font-mono)" fill="#3d3b35">{y}</text>}
              </g>
            );
          })}
        </svg>
