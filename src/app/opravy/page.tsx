  ...
  return (
    <div className="mx-auto max-w-[900px] px-5 py-12 sm:px-8 sm:py-16">
      <HlavickaStranky
        ...
        uvod={<>Každá oprava zveřejněného údaje je tady: co bylo špatně, proč a co platí teď. U jednotlivých událostí najdete totéž v části Historie.</>}
      />
      <h2 className="titul-mensi mt-16 sm:mt-20">Jak opravy děláme</h2>
      <ol className="mt-2 list-decimal space-y-1.5 pl-5 text-zaklad leading-relaxed text-tlum">
          ...
          {seznam.map((o) => {
            ...
            return (
              <li key={o.id} className="py-4">
                <div className="flex flex-wrap items-center gap-x-2 text-drobne text-tlum">
                ...
                <p className="mt-1.5 text-zaklad font-semibold text-inkoust">{o.co}</p>
                <p className="mt-1 text-zaklad leading-relaxed text-tlum">{o.proc}</p>
              </li>
            );
          })}
        </ol>
      ) : <p className="mt-3 text-zaklad text-tlum">Zatím žádná zveřejněná oprava.</p>}
      <p className="mt-8 text-male text-tlum">
        Našli jste chybu? Napište ji formulářem „Chybí tu nějaká událost?“ v <Link href="/udalosti/" className="odkaz">Událostech</Link>.
      </p>
    </div>
  );
}
