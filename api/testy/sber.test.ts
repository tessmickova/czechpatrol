...
describe("kdy se sbírá", () => {
  it("v celou a v půl, protože tik chodí po deseti minutách", () => {
    const sbira = [0, 10, 20, 30, 40, 50].filter((m) => maSeSbirat(cas(9, m)));
    expect(sbira).toEqual([0, 30]);
  });

  it("zpožděný tik okno nemine", () => {
    // Kvůli pár sekundám zpoždění nechceme sběr vynechat na celou půlhodinu.
    expect(maSeSbirat(cas(9, 1))).toBe(true);
    expect(maSeSbirat(cas(9, 31))).toBe(true);
  });
...
describe("kopnutí do sběru", () => {
  it("pošle dispatch na správný workflow a větev", async () => {
    ...
    await expect(kopniDoSberu(prostredi(), cas(9, 30))).resolves.toEqual({ spusteno: true });
  ...
  it("mimo okno se na GitHub vůbec nesáhne", async () => {
    ...
    await expect(kopniDoSberu(prostredi(), cas(9, 15))).resolves.toEqual({ spusteno: false, duvod: "není čas" });
